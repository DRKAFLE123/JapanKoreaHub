...
import { BookOpen, Search, Volume2, ChevronDown, MessageSquare, Globe, Award, ListFilter, BookCheck, X } from 'lucide-react';
import {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto font-sans space-y-5">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl">
          ...
          {getAvailableKoreanLevels().map(level => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <Award className={`w-4 h-4 ${selectedLevel === level ? 'text-white' : 'text-emerald-400'}`} />
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap pt-3 border-t border-slate-800">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              <ListFilter className="w-4 h-4 text-emerald-400" />
              <span className="text-xs font-bold text-slate-300">Select Lesson:</span>
            </div>
            ...
            <select
              ...
              className="bg-slate-950 border border-slate-800 text-xs font-bold text-white px-3 py-2 rounded-xl focus:outline-none focus:border-emerald-500 cursor-pointer"
            >
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-extrabold flex items-center gap-2 shadow-md transition-all whitespace-nowrap"
          >
            <BookCheck className="w-4 h-4" />
            <span>Lesson {selectedLesson} Korean Grammar Guide (व्याकरण)</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 mb-4 border-b border-slate-800">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 rounded-xl pl-10 pr-4 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-all"
            />
          </div>
        </div>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-1.5">
                    <div className="flex items-center gap-3 flex-wrap">
                      <div className="text-2xl font-bold font-kr text-white">
                        {vocab.word}
                      </div>
                    ...
                    {vocab.grammarSentences && vocab.grammarSentences.length > 0 && (
                      <div>
                        ...
                        {expandedGrammar === vocab.id && vocab.grammarSentences.map((gs, idx) => (
                          <div key={idx} className="mt-2 p-3 rounded-xl bg-slate-900/80 border border-emerald-900/50 space-y-1.5">
                            <div className="flex items-start gap-2">
                              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mt-0.5 whitespace-nowrap">KR</span>
                              <div className="flex-1">
                                <div className="text-sm font-kr font-bold text-white">{gs.korean}</div>
                                <div className="text-[11px] text-slate-400 italic">{gs.romanization}</div>
                              </div>
                              <button
                                onClick={() => playPronunciation(gs.korean)}
                                className="p-1 rounded bg-slate-800 hover:bg-emerald-600 text-slate-400 hover:text-white transition-all"
                              >
                  ...
                  <div className="flex-shrink-0">
                    <span className="text-[10px] px-2 py-1 rounded-lg font-black border bg-emerald-950/80 border-emerald-800 text-emerald-300">
          ...
          <div className="w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-5 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-slate-800 hover:bg-emerald-600 text-slate-400 hover:text-white transition-all"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
                    ...
                    <div className="space-y-2 pt-2 border-t border-slate-900">
                      ...
                      {point.examples.map((ex, eIdx) => (
                        <div key={eIdx} className="bg-slate-900/80 p-3 rounded-xl border border-slate-800 space-y-1">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-kr font-bold text-white">{ex.target}</span>
                            <button
