...
import { BookOpen, Search, HelpCircle, Sparkles, Volume2, ChevronDown, ChevronRight, MessageSquare, Globe, Award, BookCheck, X, Layers, ExternalLink, FileText, Printer, Music, Headphones, Mic2, Square } from 'lucide-react';
...
export const VocabularyExplorer: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto font-sans">
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-5 shadow-xl mb-4">
        <div className="flex items-center gap-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-rose-400 mb-3">
      ...
      <div className="flex gap-4 items-start">
        ...
        <aside className="hidden lg:flex flex-col w-64 xl:w-72 flex-shrink-0 sticky top-4">
          <div className="bg-slate-900/95 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden">
            {/* Sidebar header */}
            <div className="px-4 py-3.5 border-b border-slate-800 bg-slate-950/60">
              <div className="flex items-center gap-2">
                <BookOpen className="w-4 h-4 text-rose-400" />
                <span className="text-xs font-black text-white uppercase tracking-wider">Lesson Navigator</span>
              </div>
              <div className="text-[10px] text-slate-400 mt-0.5">
                {availableLessons.length} lessons • JLPT {selectedLevel}
              </div>
            </div>

            {/* Scrollable lesson list */}
            <div className="overflow-y-auto max-h-[calc(100vh-200px)] scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent py-2">
              {availableLessons.map((n) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-3 flex items-start gap-3 transition-all group border-l-[3px] ${
                      isActive
                        ? 'bg-rose-600/15 border-l-rose-500 text-white'
                        : 'border-l-transparent text-slate-400 hover:bg-slate-800/60 hover:text-white hover:border-l-slate-600'
                    }`}
                  >
                    {/* Lesson number badge */}
                    <span className={`flex-shrink-0 w-7 h-7 rounded-xl flex items-center justify-center text-[11px] font-black transition-all ${
                      isActive
                    ...
                    <div className="min-w-0 flex-1">
                      <div className={`text-[11px] font-bold leading-snug truncate ${
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          {/* Mobile: horizontal lesson strip (shown only on small screens) */}
          <div className="lg:hidden flex items-center gap-2 overflow-x-auto scrollbar-none bg-slate-900/90 border border-slate-800 rounded-2xl p-3">
            <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap">Lesson:</span>
            {availableLessons.map((n) => (
              <button
                key={n}
                onClick={() => {
                ...
                className={`px-2.5 py-1.5 rounded-xl text-[11px] font-bold whitespace-nowrap border transition-all flex-shrink-0 ${
          ...
          {/* ── Lesson Header: Title + Audio controls + Action buttons ── */}
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-xl">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                ...
                <h2 className="text-base sm:text-lg font-black text-white mt-0.5">
                  {searchQuery
                    ? 'Search Results'
                    : `Lesson ${selectedLesson}: ${LESSON_TOPICS[selectedLesson] || ''}`}
                </h2>
              </div>

              <div className="flex items-center gap-2 flex-wrap">
                {/* N5 Audio Track Buttons */}
                {selectedLevel === 'N5' && (() => {
                  ...
                  const btns: { type: 'vocab' | 'dialogue' | 'drill'; label: string; labelJp: string; url: string; icon: React.ReactNode; color: string; activeColor: string }[] = [
                    { type: 'vocab',    label: 'Vocabulary', labelJp: '単語', url: tracks.vocab,    icon: <Music className="w-3.5 h-3.5" />,     color: 'bg-violet-700 hover:bg-violet-600',   activeColor: 'bg-violet-500 ring-2 ring-violet-300' },
                    { type: 'dialogue', label: 'Dialogue',   labelJp: '会話', url: tracks.dialogue, icon: <Headphones className="w-3.5 h-3.5" />, color: 'bg-emerald-700 hover:bg-emerald-600', activeColor: 'bg-emerald-500 ring-2 ring-emerald-300' },
                    { type: 'drill',    label: 'Drills',     labelJp: '練習', url: tracks.drill,    icon: <Mic2 className="w-3.5 h-3.5" />,      color: 'bg-sky-700 hover:bg-sky-600',         activeColor: 'bg-sky-500 ring-2 ring-sky-300' },
                  ];
                  return (
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-bold text-slate-400 hidden sm:block">🎵</span>
                      {btns.map((btn) => (
                        <button
                          key={btn.type}
                          onClick={() => playLessonTrack(btn.type, btn.url)}
                          title={`${btn.label} — ${btn.labelJp}`}
                      ...
                      {playingTrack && (
                        <button onClick={stopAudio} title="Stop" className="p-1.5 rounded-lg bg-rose-700 hover:bg-rose-600 text-white transition-all">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-extrabold flex items-center gap-1.5 shadow-md transition-all whitespace-nowrap"
                >
                  <FileText className="w-3.5 h-3.5" />
                  <span className="hidden sm:inline">Vocab Note</span>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white text-xs font-extrabold flex items-center gap-1.5 shadow-md transition-all whitespace-nowrap"
                >
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl sm:rounded-3xl p-3.5 sm:p-6 shadow-2xl">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-3 mb-3 border-b border-slate-800">
              <div className="text-xs font-semibold text-slate-400">
                {filteredVocab.length} word{filteredVocab.length !== 1 ? 's' : ''} in {searchQuery ? 'search' : `Lesson ${selectedLesson}`}
              </div>
