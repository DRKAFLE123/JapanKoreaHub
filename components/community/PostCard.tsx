...
export default function PostCard({
  ...
  const [sharesCount, setSharesCount] = useState(
    post.sharesCount ?? Math.max(1, Math.floor((post.likesCount || 6) / 3))
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs hover:shadow-md hover:border-slate-300 transition-all duration-200 overflow-hidden flex flex-col group">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide ${
              isJob
                ? isJapan ? 'bg-red-50 text-red-700 border border-red-200/70' : 'bg-blue-50 text-blue-700 border border-blue-200/70'
        ...
        <h3
          ...
          className="text-[15px] sm:text-base font-semibold text-slate-900 leading-snug group-hover:text-indigo-600 transition-colors cursor-pointer mb-1.5 line-clamp-2"
        >
          {post.title}
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 my-2 p-2.5 rounded-xl bg-slate-50/80 border border-slate-200/60">
          <div>
          ...
          <div>
            ...
            {post.serviceCharge === 0 ? (
              <div className="inline-flex items-center gap-1 mt-0.5 px-2 py-0.5 rounded-md bg-emerald-50 border border-emerald-200/60 text-emerald-700 text-xs font-medium">
                <Sparkles className="w-3 h-3 text-emerald-600" />
                <span>Free (¥0 / ₩0)</span>
              </div>
            ) : (
              <div className="inline-flex items-center gap-1 mt-0.5 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200/60 text-amber-800 text-xs font-medium">
