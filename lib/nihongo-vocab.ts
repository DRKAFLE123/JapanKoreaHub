...
export interface VocabItem {
  ...
  romaji?: string;        // English / Romaji pronunciation
...
// ============================================================
// KANA TO ROMAJI CONVERTER ENGINE
// Converts Hiragana/Katakana to clean English Romaji pronunciation
// ============================================================
const KANA_ROM_MAP: Record<string, string> = {
  // Hiragana Basic & Dakuten
  'あ': 'a', 'い': 'i', 'う': 'u', 'え': 'e', 'お': 'o',
...
export function kanaToRomaji(input: string): string {
  ...
  while (i < input.length) {
    ...
    // Check Yoon 2-char pair first
    if (YOON_MAP[twoChars]) {
      converted = YOON_MAP[twoChars];
      i += 2;
    } else if (KANA_ROM_MAP[char]) {
      converted = KANA_ROM_MAP[char];
      i++;
    } else {
      converted = char;
      i++;
    ...
    if (doubleConsonant) {
      if (converted.length > 0 && /[bcdfghjklmnpqrstvwxyz]/i.test(converted[0])) {
        result += converted[0];
      }
