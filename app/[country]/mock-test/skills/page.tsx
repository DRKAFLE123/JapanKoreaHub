...
export default async function SkillsMockTestPage({ params, searchParams }: Props) {
  const resolvedParams = await params;
  const resolvedSearchParams = searchParams ? await searchParams : undefined;

  const validCountries = ['japan', 'korea'];
  if (!validCountries.includes(resolvedParams.country)) {
    notFound();
  }

  return (
    <SkillsExamDirectoryClient
      country={resolvedParams.country as 'japan' | 'korea'}
