...
export async function generateMetadata({ params }: { params: Promise<{ country: string }> }): Promise<Metadata> {
  ...
  const titles: Record<string, string> = {
    japan: 'JLPT & JFT-Basic CBT Mock Tests — JapanKoreaHub',
    korea: 'EPS-TOPIK & TOPIK Mock Tests — JapanKoreaHub',
  };
  return {
    title: titles[country] ?? 'Mock Tests — JapanKoreaHub',
    description: country === 'japan'
      ? 'Interactive JLPT N5–N1 and JFT-Basic listening & reading CBT mock test simulators with timer, auto-grading, and full Nepali answer analysis.'
      : 'Interactive EPS-TOPIK and TOPIK I & II CBT/UBT mock test simulators with official timing and instant scorecards.',
    alternates: { canonical: `https://japankoreahub.com/${country}/mock-test` },
  };
}

export default async function MockTestPage({ params }: { params: Promise<{ country: string }> }) {
  const { country } = await params;
