...
clean_vocab_ts = re.sub(r"\s*\{\s*id:'v51_1'.*?\n", "", vocab_ts)
clean_vocab_ts = re.sub(r"\s*\{\s*id:'v53_1'.*?\n", "", clean_vocab_ts)

array_end = clean_vocab_ts.find("];\n\nexport function getVocabByLevel")
if array_end != -1:
    vocab_entries_json = []
    for item in n3_vocab_lessons_51_60:
        item_str = "  " + json.dumps(item, ensure_ascii=False)
        vocab_entries_json.append(item_str)
    
    new_vocab_code = clean_vocab_ts[:array_end].rstrip() + ",\n\n  // ════════════════════════════════════════════════════════\n  // LESSONS 51 TO 60 — N3 COMPLETE VOCABULARY\n  // ════════════════════════════════════════════════════════\n" + ",\n".join(vocab_entries_json) + "\n" + clean_vocab_ts[array_end:]
    
    with open("lib/nihongo-vocab.ts", "w", encoding="utf-8") as f:
        f.write(new_vocab_code)
    print("✅ lib/nihongo-vocab.ts updated with Lessons 51-60 vocabulary!")
...
n3_section_pos = grammar_ts.find("// N3 (Lesson 51)")
if n3_section_pos != -1:
    korean_pos = grammar_ts.find("// KOREAN GRAMMAR LESSONS")
    if korean_pos != -1:
        close_bracket = grammar_ts.rfind("];", 0, korean_pos)
        prefix = grammar_ts[:n3_section_pos].rstrip()
        suffix = grammar_ts[close_bracket:]
        
        grammar_entries_json = []
        for g_guide in n3_grammar_lessons_51_60:
            g_str = "  " + json.dumps(g_guide, ensure_ascii=False, indent=2).replace("\n", "\n  ")
            grammar_entries_json.append(g_str)
        
        n3_grammar_code = prefix + "\n\n  // ════════════════════════════════════════════════════════\n  // N3 COMPLETE — LESSONS 51 TO 60\n  // ════════════════════════════════════════════════════════\n,\n" + ",\n".join(grammar_entries_json) + "\n" + suffix
        
        with open("lib/grammar-guide.ts", "w", encoding="utf-8") as f:
            f.write(n3_grammar_code)
        print("✅ lib/grammar-guide.ts updated with Lessons 51-60 grammar guides!")

# ─────────────────────────────────────────────────────────────────
# UPDATE lib/kanji-dataset.ts
# ─────────────────────────────────────────────────────────────────
with open("lib/kanji-dataset.ts", "r", encoding="utf-8") as f:
    kanji_ts = f.read()

array_close_pos = kanji_ts.rfind("];")
if array_close_pos != -1:
    kanji_entries_json = []
    for k_item in n3_kanji_lessons_51_60:
        k_str = "  " + json.dumps(k_item, ensure_ascii=False)
        kanji_entries_json.append(k_str)
    
    new_kanji_code = kanji_ts[:array_close_pos].rstrip() + ",\n\n  // ════════════════════════════════════════════════════════\n  // N3 KANJI — LESSONS 51 TO 60\n  // ════════════════════════════════════════════════════════\n" + ",\n".join(kanji_entries_json) + "\n" + kanji_ts[array_close_pos:]
    
    with open("lib/kanji-dataset.ts", "w", encoding="utf-8") as f:
        f.write(new_kanji_code)
    print("✅ lib/kanji-dataset.ts updated with Lessons 51-60 Kanji!")

print("\n🎉 ALL LESSONS 51-60 (VOCABULARY, GRAMMAR, KANJI) SUCCESSFULLY COMPLETED!")
