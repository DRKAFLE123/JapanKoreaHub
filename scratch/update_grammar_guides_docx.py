...
def parse_examples(ex_str):
    ...
    for part in parts:
        ...
        m = re.match(r'^(.*?)\s*[\(\（](.*?)[\)\）]$', part)
        if m:
            jp = m.group(1).strip().replace("'", "\\'")
            eng = m.group(2).strip().replace("'", "\\'")
            np_val = get_example_nepali(jp, eng).replace("'", "\\'")
            parsed.append({
                "target": jp,
                "reading": jp,
                "english": eng,
...
# Generate TypeScript for Lessons 13-20 Grammar Guides
lesson_titles = {
    13: 'Desires & Purpose of Movement (欲しい・〜たい・目的)',
    14: 'Te-Form Conjugation & Polite Requests (て形と依頼)',
    15: 'Permission, Prohibition & States (許可・禁止・状態)',
    16: 'Connecting Sentences & Actions in Sequence (文の接続)',
    17: 'Nai-Form, Instructions & Must-Do Rules (ない形と義務)',
    18: 'Dictionary Form & Expressing Ability/Hobbies (辞書形と可能)',
    19: 'Ta-Form & Past Experiences (た形と経験)',
    20: 'Casual & Plain Speech Style (普通体と会話)'
}

def generate_grammar_ts():
    lines = []
    for l in range(13, 21):
        g_raw = docx_data[str(l)]["grammar"]
        l_title = lesson_titles[l].replace("'", "\\'")
        
        lines.append(f"  // ────────────────────────────────────────────────────────────")
        lines.append(f"  // LESSON {l}")
        lines.append(f"  // ────────────────────────────────────────────────────────────")
        lines.append(f"  {{\n    language: 'JAPANESE', level: 'N5', lesson: {l},")
        lines.append(f"    lessonTitle: '{l_title}',")
        lines.append(f"    grammarPoints: [")
        
        for gp in g_raw:
            title = gp["title"].replace("'", "\\'")
            eng_exp = gp["explanation"].replace("'", "\\'")
            np_exp = get_grammar_nepali_explanation(title, eng_exp).replace("'", "\\'")
            examples = parse_examples(gp["examples"])
            
            ex_lines = []
            for ex in examples:
                ex_lines.append(
                    f"          {{ target: '{ex['target']}', reading: '{ex['reading']}', english: '{ex['english']}', nepali: '{ex['nepali']}' }}"
                )
            ex_str = ",\n".join(ex_lines)
            ...
            gp_code = (
                ...
                f"        title: '{title}',\n"
                f"        pattern: '{title}',\n"
                f"        explanationEnglish: '{eng_exp}',\n"
                f"        explanationNepali: '{np_exp}',\n"
                f"        examples: [\n{ex_str}\n        ]\n"
                f"      }}"
            )
            lines.append(gp_code + ",")
            
        lines.append(f"    ]\n  }},")
        
    return "\n".join(lines)

# Read lib/n5-grammar-guides.ts and inject Lessons 13-20
with open("lib/n5-grammar-guides.ts", "r", encoding="utf-8") as f:
    content = f.read()

# Split before Lesson 13 and after Lesson 20
pos_l13 = content.find("// LESSON 13")
pos_l21 = content.find("// LESSON 21")

if pos_l13 != -1 and pos_l21 != -1:
    header = content[:pos_l13].rstrip()
    footer = content[pos_l21:].lstrip()
    new_content = header + "\n" + generate_grammar_ts() + "\n\n  " + footer
    with open("lib/n5-grammar-guides.ts", "w", encoding="utf-8") as f:
        f.write(new_content)
    print("Updated lib/n5-grammar-guides.ts with official DOCX grammar data!")
else:
    print("Error: Could not locate Lesson 13 / Lesson 21 split points in lib/n5-grammar-guides.ts")
